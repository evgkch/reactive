...
export class ListImpl<T> extends Reactive<ListPatch<T>> {
    ...
    constructor(initial: T[] = []) {
        ...
        p.pop = () => {
            ...
            const patch: ListPatch<T> = { start: arr.length, removed: [val], added: [] };
            core.trigger(arr, LENGTH_KEY, patch);
            return val;
        };
        ...
        p.sort = (fn?: (a: T, b: T) => number) => {
            ...
            const patch: ListPatch<T> = { start: 0, removed: before, added: [...arr] };
            core.trigger(arr, REORDER_KEY, patch);
            return proxy;
        };
        ...
        p.copyWithin = (target: number, start: number, end?: number) => {
            const len = end !== undefined ? Math.min(end, arr.length) - start : arr.length - start;
            ...
            const patch: ListPatch<T> = { start: target, removed: prev, added: copied };
            core.trigger(arr, REORDER_KEY, patch);
            return proxy;
        };
